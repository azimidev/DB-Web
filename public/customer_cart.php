	...
	<section class="main col-lg-8">
		...
		<table class="table table-hover">
			...
			<?php while($cart = mysqli_fetch_assoc($cart_set)): ?>
				<tr>
					<td><?php echo htmlentities(find_performance_by_id($cart["performance_id"])["performance_name"]); ?></td>
					<td>
						<a href="../includes/layouts/add_to_booking.php?performance=<?php echo urldecode(find_performance_by_id($cart["performance_id"])["performance_id"]); ?>&cart=<?php echo urldecode($cart["id"]); ?>" class="btn btn-success btn-xs">
							<span class="glyphicon glyphicon-plus"></span>
						</a>
						<a href="delete_cart_item.php?id=<?php echo urldecode($cart["id"]); ?>" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure you want to cancel booking for <?php echo htmlentities(find_performance_by_id($cart["performance_id"])["performance_name"]); ?> performance?')">
